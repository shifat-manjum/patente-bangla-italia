...
export const RoundsCurriculumView: React.FC<RoundsCurriculumViewProps> = ({
  ...
  return (
    <div className="space-y-6 pb-24 md:pb-12">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-[#12161F] border border-white/10 text-sm text-white placeholder-slate-500 focus:outline-hidden focus:border-white/30 focus:ring-1 focus:ring-white/20 transition shadow-sm"
          />
        </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition cursor-pointer ${
                filterTab === tab.id
                  ...
                  : 'bg-[#12161F] text-slate-400 hover:text-white hover:bg-[#181F2C] border border-white/10'
              }`}
            >
              {tab.label}
            </button>
      ...
      {/* Modern 240 Rounds Grid (Clean Obsidian Cards with Italian First, then Bangla) */}
          ...
          return (
            <div
              ...
              className={`rounded-2xl border p-5 sm:p-5.5 transition-all duration-200 transform hover:-translate-y-1 hover:shadow-xl relative flex flex-col justify-between min-h-[225px] ${
                isCurrent
                  ? 'bg-[#161C28] border-white/40 ring-1 ring-white/30 shadow-[0_0_20px_rgba(229,46,45,0.25)]'
                  : isPassed
                  ? 'bg-[#12161F] border-white/20 hover:border-white/30'
                  : isUnlocked
                  ? 'bg-[#12161F] border-white/10 hover:border-white/25 shadow-sm'
                  : 'bg-[#0E121A] border-white/5 opacity-80'
              }`}
            >
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  {/* Status Indicator */}
                  {isPassed ? (
                    <span className="flex items-center gap-1 text-[11px] font-black text-emerald-400 bg-emerald-950/40 px-2.5 py-1 rounded-full border border-emerald-500/30">
                      <CheckCircle2 className="w-3.5 h-3.5" />
                      <span>{30 - (round.result?.errors ?? 0)}/৩০ পাস</span>
                    </span>
                  ) : !isUnlocked ? (
                    <span className="flex items-center gap-1 text-[11px] font-bold text-slate-400 bg-white/5 px-2.5 py-0.5 rounded-full border border-white/10">
                      <Lock className="w-3 h-3 text-slate-400" />
                      <span>{isPaidSyllabus ? '€৪৯ একাডেমি' : 'লক করা'}</span>
                    </span>
                  ) : isCurrent ? (
                    <span className="flex items-center gap-1.5 text-[11px] font-black text-white bg-white/10 px-2.5 py-1 rounded-full border border-white/20 animate-pulse">
                      <span className="w-2 h-2 rounded-full bg-[#E52E2D]" />
                      <span>রানিং রাউন্ড</span>
                    </span>
                  ) : (
                    <span className="text-[10px] font-bold text-slate-400 bg-white/5 px-2.5 py-0.5 rounded-full border border-white/10">
                ...
                <div className="pt-1 space-y-1">
                  ...
                  <p className="text-xs sm:text-[13px] font-bold text-slate-300 pt-0.5 leading-snug">
                    {round.titleBn}
                  </p>
                </div>

                {/* Exam Specs (Bangla) */}
                <div className="flex items-center gap-1.5 flex-wrap pt-1 text-[10.5px] font-semibold text-slate-400">
              ...
              <div className="pt-4 border-t border-white/10 mt-3">
                {isPaidSyllabus ? (
                  ...
                  <button
                    ...
                    className="w-full py-2.5 px-3 rounded-full bg-white/5 hover:bg-white/10 text-slate-400 font-bold text-xs flex items-center justify-center gap-1.5 border border-white/10 transition cursor-pointer"
                  ...
                  <button
                    ...
                    className={`w-full py-2.5 px-3 rounded-full font-black text-xs flex items-center justify-center gap-1.5 transition cursor-pointer shadow-md active:scale-95 ${
                      isCurrent
                        ? 'bg-white text-slate-950 hover:bg-slate-100 shadow-md ring-2 ring-white/30'
                        : isPassed
                        ? 'bg-white/10 hover:bg-white/15 text-white border border-white/20'
                        : 'bg-white text-slate-950 hover:bg-slate-100'
                    }`}
                  >
                    <Play className={`w-3.5 h-3.5 fill-current ${isCurrent || !isPassed ? 'text-slate-950' : 'text-white'}`} />
                    <span>
                      {isCurrent 
