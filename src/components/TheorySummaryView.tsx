        ...
        {filteredChapters.map((chapter: TheoryChapter) => (
          ...
          >
            ...
            <div className="p-4 sm:p-6 space-y-5">
              {/* Summary Description */}
              <p className="text-xs sm:text-sm text-slate-600 font-medium leading-relaxed bg-slate-50 p-3 rounded-xl border border-slate-100">
                💡 <span className="font-bold text-slate-800">Chapter Overview:</span> {chapter.summaryBn}
              </p>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2.5">
                  {chapter.goldenRules.map((rule, idx) => (
                    <div
                      key={idx}
                      className="p-3.5 rounded-xl bg-blue-50/40 border border-blue-100 space-y-1.5 relative group"
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-white hover:bg-blue-100 text-blue-700 border border-blue-200 transition shrink-0 cursor-pointer"
                      ...
                      <p className="text-xs sm:text-sm text-slate-700 font-medium pl-6 leading-relaxed">
                        👉 <span className="font-semibold text-slate-800">সহজ বাংলা অর্থ:</span> {rule.bn}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              {chapter.trapKeywords.length > 0 && (
                <div className="space-y-2.5 pt-2 border-t border-slate-100">
                  <div className="flex items-center gap-1.5 text-xs font-black uppercase tracking-wider text-rose-700">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {chapter.trapKeywords.map((trap, idx) => (
                      <div
                        key={idx}
                        className="p-3 rounded-xl bg-amber-50/60 border border-amber-200/80 space-y-1"
                      >
                        <div className="flex items-center justify-between gap-1">
                          <span className="text-xs font-black text-rose-800 bg-rose-100 px-2 py-0.5 rounded-md">
                            ⚠️ {trap.word}
                          </span>
            ...
            <div className="px-4 py-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
              <span>Related Practice: <strong>Round #{chapter.relatedRounds.join(', #')}</strong></span>
              <button
                type="button"
                onClick={() => onStartRound(chapter.relatedRounds[0] || 1)}
